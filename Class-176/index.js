...
//jquery is represented by $

$(document).ready(function(){ //ready() acts as a init()
    displayStory()
})

$(function(){
    $(".input_field").keyup(function(){
        let id=$(this).attr("id") //attr- attribute
        let input_number = id.split("_")[1]

        $(".rep_input").eq(input_number).html($(this).val()) //val - gets the value of the field
    })
...
function displayStory(){
    ...
    for(let i=0; i<story.inputs; i++){
        let input_html = `<input type="text" class="input_field" id="input_${i}" 
